...
from account.models import User, SiteViewLog
from product.models import Product, ProductVariant, Category, Review
from sales.models import Sales, Saled_Products
...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        # ============ SALES STATS ============
        # Filter by range
        filtered_sales = Sales.objects.exclude(status__in=['unpaid', 'cancelled']).filter(
            created__date__gte=start_date,
            created__date__lte=end_date
        )
        
        prev_sales = Sales.objects.exclude(status__in=['unpaid', 'cancelled']).filter(
            created__date__gte=prev_start_date,
            created__date__lte=prev_end_date
        )
        
        # Revenue metrics
        total_revenue = filtered_sales.aggregate(total=Sum('total_amt'))['total'] or 0
        prev_total_revenue = prev_sales.aggregate(total=Sum('total_amt'))['total'] or 0
        ...
        processing_orders = filtered_sales.filter(status__in=['verified', 'proceed', 'packed']).count()
        delivered_orders = filtered_sales.filter(status='delivered').count()
        successful_orders = filtered_sales.filter(status='successful').count()
        ...
        # Average order value
        avg_order_value = filtered_sales.aggregate(avg=Avg('total_amt'))['avg'] or 0
...
class SalesChartView(APIView):
    ...
    def get(self, request):
        ...
        if days_diff > 366: # Data points aggregation strategy needed for large ranges, but for now just raw
            pass 
        ...
        while current_date <= end_date:
            if current_date in sales_by_date:
                daily_data.append({
                    ...
                    'revenue': round(sales_by_date[current_date]['revenue'] or 0, 2),
        ...
        # Sales by status for selected period
        status_data = Sales.objects.filter(
            created__date__gte=start_date,
            created__date__lte=end_date
...
class TopProductsView(APIView):
    ...
    def get(self, request):
        ...
        top_products = Saled_Products.objects.filter(
            product__isnull=False
        ).values(
            'product__id',
            'product__product_name',
            'product__productslug'
        ).annotate(
            total_qty=Sum('qty'),
...
class RecentOrdersView(APIView):
    ...
    def get(self, request):
        ...
        orders = Sales.objects.select_related('costumer_name').order_by('-created')[:limit]
        ...
        for order in orders:
            recent.append({
                ...
                'customer': f"{order.costumer_name.first_name} {order.costumer_name.last_name}" if order.costumer_name else "Guest",
...
class VisitorStatsView(APIView):
    ...
    def get(self, request):
        ...
        daily_views = SiteViewLog.objects.filter(
        ...
        by_country = SiteViewLog.objects.filter(
        ...
        by_device = SiteViewLog.objects.filter(
            timestamp__date__gte=last_30_days
...
class CategoryPerformanceView(APIView):
    ...
    def get(self, request):
        # Sales by category
        category_sales = Saled_Products.objects.filter(
