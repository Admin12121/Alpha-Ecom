...
class ProductVariant(models.Model):
    ...
    def clean(self):
        ...
        if not self.color_code and not self.size:
            raise ValidationError({'color_code': 'Provide a color_code or a size for the variant.'})

        if self.color_code:
            normalized = self._normalize_color(self.color_code)
            if not re.match(r'^#[0-9A-F]{6}$', normalized):
                raise ValidationError({'color_code': 'Invalid color code. Use #RRGGBB.'})
            self.color_code = normalized
        ...
        if self.color_code and self.color and self.color.color_code != self.color_code:
            raise ValidationError({'color': 'Color reference does not match color_code.'})
...
class NotifyUser(models.Model):
    ...
    variant = models.ForeignKey(ProductVariant, on_delete=models.SET_NULL, null=True, blank=True)  # was SmallIntegerField
...
class Cart(models.Model):
    ...
    variant = models.ForeignKey(ProductVariant, on_delete=models.SET_NULL, null=True, default=None, related_name='cart_product_variant')
