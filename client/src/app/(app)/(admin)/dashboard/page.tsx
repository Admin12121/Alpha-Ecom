...
const STATUS_COLORS: Record<string, string> = {
  ...
  proceed: "text-indigo-600 bg-indigo-100 dark:bg-indigo-900/30 dark:text-indigo-400",
  packed: "text-purple-600 bg-purple-100 dark:bg-purple-900/30 dark:text-purple-400",
  delivered: "text-green-600 bg-green-100 dark:bg-green-900/30 dark:text-green-400",
  successful: "text-green-600 bg-green-100 dark:bg-green-900/30 dark:text-green-400",
  ...
  in_progress: "text-indigo-600 bg-indigo-100 dark:bg-indigo-900/30 dark:text-indigo-400",
...
export default function DashboardPage() {
  ...
  const { data: stats, isLoading: statsLoading } = useGetDashboardStatsQuery({ token, ...dateParams }, { skip });
  const { data: salesChart, isLoading: salesChartLoading } = useGetSalesChartQuery({ token, ...dateParams }, { skip });
  const { data: topProducts, isLoading: topProductsLoading } = useGetTopProductsQuery({ token, limit: 5 }, { skip }); // Top products usually all time or recent, keep as is unless requested
  const { data: recentOrders, isLoading: recentOrdersLoading } = useGetRecentOrdersQuery({ token, limit: 7 }, { skip });
  const { data: recentBookings, isLoading: recentBookingsLoading } = useGetRecentBookingsQuery({ token, limit: 5 }, { skip });
  const { data: visitorStats, isLoading: visitorStatsLoading } = useGetVisitorStatsQuery({ token }, { skip });
  const { data: categoryPerf, isLoading: categoryPerfLoading } = useGetCategoryPerformanceQuery({ token }, { skip });

  // Prepare chart data
  const statusData = salesChart?.by_status?.map((item: any, index: number) => ({
    ...item,
    fill: `hsl(var(--chart-${(index % 5) + 1}))`
  })) || [];

  const totalRevenue = React.useMemo(() => {
    return salesChart?.daily?.reduce((acc: any, curr: any) => acc + curr.revenue, 0) || 0
  }, [salesChart?.daily]);
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
            ...
            <Card className="col-span-4 lg:col-span-5">
              <CardHeader>
                ...
                <CardDescription>
                  Revenue for {dateRange === 'all' ? 'All Time' :
                    dateRange === 'monthly' ? 'this month' :
                      dateRange === 'yearly' ? 'this year' : 'last 6 months'}
                </CardDescription>
              </CardHeader>
              <CardContent className="pl-0">
                {salesChartLoading ? (
                  ...
                  <ChartContainer config={revenueChartConfig} className="aspect-auto h-[350px] w-full">
                    <AreaChart
            ...
            <Card className="col-span-4 lg:col-span-2 flex flex-col">
              ...
              <CardFooter className="flex-col gap-2 text-sm">
                <div className="flex items-center gap-2 font-medium leading-none">
                  Most orders are {stats?.pending_orders > 0 ? "Pending" : "Completed"} <TrendingUp className="h-4 w-4" />
                </div>
                          ...
                          <TableRow key={order.id}>
                            ...
                            <TableCell>
                              <div className="flex flex-col">
                                ...
                                <span className="text-xs text-muted-foreground hidden sm:inline">{order.email}</span>
                              </div>
                            </TableCell>
                            <TableCell>
                      ...
                      topProducts?.products.map((product: any) => (
                        <div key={product.id} className="flex items-center">
                          <Avatar className="h-9 w-9 rounded-md border">
                            <AvatarImage src={product.image} alt={product.name || "Product"} />
          ...
          <div className="grid gap-4 md:grid-cols-1">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Latest appointments from customers</CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      {recentBookings?.bookings.length === 0 ? (
                        ...
                        recentBookings?.bookings.map((booking: any) => (
                          <TableRow key={booking.id}>
                            ...
                            <TableCell>
                              {booking.date}
                              <span className="text-xs text-muted-foreground ml-2">{booking.time}</span>
                            </TableCell>
                            <TableCell>
                              <Badge variant="outline" className={STATUS_COLORS[booking.status] || "bg-gray-100"}>
              ...
              <CardContent>
                {categoryPerfLoading ? (
                  ...
                  <ChartContainer config={categoryChartConfig} className="min-h-[200px] w-full">
                    <BarChart accessibilityLayer data={categoryPerf?.categories || []} layout="vertical" margin={{ left: 0 }}>
            ...
            <Card>
              ...
              <CardContent>
                {visitorStatsLoading ? (
                  ...
                  <ChartContainer config={visitorsChartConfig} className="min-h-[200px] w-full">
                    <LineChart
                      accessibilityLayer
